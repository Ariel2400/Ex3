...
void BMP::init(const BMP &other) {
  ...
  bmp_color_palette.color_list =
      std::make_unique<Matrix>(*other.bmp_color_palette.color_list);
  bmp_color_palette.pixel_color_indexes =
      std::make_unique<Matrix>(*other.bmp_color_palette.pixel_color_indexes);
  pixels = std::make_unique<Matrix>(*other.pixels);
}

BMP::BMP() {}

BMP::BMP(const BMP &other) { init(other); }
...
bool BMP::read(const std::string fname) {
  ...
  if (inp) {
    ...
    if (bmp_info_header.bit_count == 24) {
      ...
      std::cerr << "The program can treat only 24 or 8 bits per pixel BMP files"
                << std::endl;
      inp.close();
      return false;
    }
...
void BMP::rotate_image() {
  ...
  // pixel (i,j) in the new image corresponds to pixel (j,height-i-1) in the
  // existing image
  std::unique_ptr<Matrix> rotated_pixels = std::make_unique<Matrix>(
      bmp_info_header.height, bmp_info_header.width * COLORS_IN_A_PIXEL);
  for (int i = 0; i < bmp_info_header.height; ++i) {
    for (int j = 0; j < bmp_info_header.width; ++j) {
      rotated_pixels->set_value(
          i, COLORS_IN_A_PIXEL * j,
          pixels->get_value(j, COLORS_IN_A_PIXEL *
                                   (bmp_info_header.height - i - 1)));
      rotated_pixels->set_value(
          i, COLORS_IN_A_PIXEL * j + 1,
          pixels->get_value(
              j, COLORS_IN_A_PIXEL * (bmp_info_header.height - i - 1) + 1));
      rotated_pixels->set_value(
          i, COLORS_IN_A_PIXEL * j + 2,
          pixels->get_value(
              j, COLORS_IN_A_PIXEL * (bmp_info_header.height - i - 1) + 2));
    }
  }
  pixels = std::make_unique<Matrix>(*rotated_pixels);
  if (bmp_info_header.bit_count == 8) {
    // rotate the indexes matrix
    std::unique_ptr<Matrix> rotated_pixel_color_indexes =
    ...
    bmp_color_palette.pixel_color_indexes =
        std::make_unique<Matrix>(*rotated_pixel_color_indexes);
  }
}

BMP::~BMP() {}

// calculate the minimum number that needs to be added to 'num' to make it
// divisible by 'align'
...
std::unique_ptr<Matrix> BMP::vector_to_matrix(vector<uint8_t> vector,
...
void BMP::read_24_bit(std::ifstream *inp, vector<uint8_t> data) {
  ...
  pixels = vector_to_matrix(data, bmp_info_header.height,
                            bmp_info_header.width * COLORS_IN_A_PIXEL);
}
...
void BMP::read_8_bit(std::ifstream *inp) {
  ...
  for (int i = 0; i < num_of_colors; ++i) {
    inp->read((char *)(bgr_values.data() + COLORS_IN_A_PIXEL * i),
              COLORS_IN_A_PIXEL);
    inp->read((char *)padding.data(), padding.size());
  }
  // bgr values of color number i are stored in color_list(i,0),
  // color_list(i,1), color_list(i,2)
  bmp_color_palette.color_list =
      vector_to_matrix(bgr_values, num_of_colors, COLORS_IN_A_PIXEL);
  ...
  pixels = std::make_unique<Matrix>(bmp_info_header.height,
                                    bmp_info_header.width * COLORS_IN_A_PIXEL);
  for (int i = 0; i < bmp_info_header.height; ++i) {
    for (int j = 0; j < bmp_info_header.width; ++j) {
      pixels->set_value(
...
void BMP::write_8_bit(std::ofstream &of) {
  ...
  for (int i = 0; i < num_of_colors; ++i) {
    of.write((const char *)(color_palette.data() + COLORS_IN_A_PIXEL * i),
             COLORS_IN_A_PIXEL);
    of.write((const char *)color_palette_padding.data(),
             color_palette_padding.size());
  }
